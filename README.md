...
Flutter 离线视频缓存包：无 ffmpeg、无 SQLite、**无捆绑二进制**。下载、转封装、
持久化全是 Dart；唯一的原生代码是一段几十行的平台通道，把 HLS 的 AES-128 解密
交给系统加密 API（硬件 AES 指令），系统 API 不可用时自动退回纯 Dart。

**English** — `video_cacher` is a Flutter package for offline video caching. It
downloads direct mp4 links and HLS (m3u8) streams with resumable downloads,
expired-URL refresh and AES-128 segment decryption, then remuxes the TS segments
into a local mp4 (h264 + AAC, no transcoding) inside a background isolate, and
can optionally save the result to the system photo album. No ffmpeg, no SQLite,
**no bundled binaries** — the only native code is a thin platform channel that
routes AES-128-CBC to the OS crypto library (`javax.crypto` on Android,
`CommonCrypto` on iOS) so it runs on the CPU's AES instructions, with a
pure-Dart fallback everywhere else.
...
- HLS AES-128 分片解密走**系统硬件 AES**（见下节），不卡主线程
...
加密 HLS 的解密量等于整段视频大小。纯 Dart 的 AES 在真机上实测**约 1MB/s**
（已开 3 路 isolate 并行），433MB 的加密视频光解密就要 400 秒——占整个「处理中」
阶段的 98%：
...
ARMv8 CPU 有 AES 指令，系统加密库直接用它，是 500-2000MB/s 量级。所以 0.3.0
起本包重新成为 Flutter plugin，只为把这一步交给系统：

| 平台 | 实现 | 线程 |
|---|---|---|
| Android | `javax.crypto.Cipher("AES/CBC/PKCS5Padding")`（Conscrypt → BoringSSL） | 插件内固定线程池，不占平台主线程 |
| iOS | `CommonCrypto` 的 `CCCrypt`（`kCCAlgorithmAES` + `kCCOptionPKCS7Padding`） | 后台 `DispatchQueue`，不占平台主线程 |
| 其他（macOS/桌面/测试） | 无原生实现 | 自动退回纯 Dart（pointycastle） |

关键点：

- **不捆绑任何二进制**：没有 `.so`、没有 `.xcframework`、没有 vendored
  framework，原生侧就是一个 Kotlin 文件 + 一个 Swift 文件，只调用系统 API，
  对包体积的影响可以忽略。
- **永远有纯 Dart 兜底**：主 isolate 启动 remux 前用一个固定向量做一次能力
  探测（同时校验 PKCS7 去填充语义一致），探测不过就整轮走 pointycastle；
  单片解密失败也只退回这一片，不影响其余分片。
- **产物逐字节一致**：硬件路径与纯 Dart 路径的明文必须完全相同，仓库里有
  单测（纯 Dart 路径 sha256 固定）与真机 integration test（两条路径逐字节比对）
  各自把关。
...
> 升级注意：0.3.0 新增了原生代码，**必须完整重建**（`flutter clean` 后重装），
> 热重载/增量安装不会带上新插件。
...
本包是 Flutter plugin（只为硬件 AES，见上文），无需任何额外配置：Android 侧
最低 `minSdk 21`，iOS 侧最低 `12.0`，都不引入第三方依赖。

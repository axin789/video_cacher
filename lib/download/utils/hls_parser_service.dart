...
class HlsParserService {
  ...
  Future<HlsParsedResult> parseFromEntryUrl(String entryUrl) async {
    ...
    final entryText = (await dio.get<String>(entryUrl,
    ...
    if (entryPlaylist is HlsMasterPlaylist) {
      if (entryPlaylist.variants.isEmpty) {
        throw StateError('Master playlist has no variants.');
      }
    ...
    final mediaText = (await dio.get<String>(mediaUrl,
                options: Options(responseType: ResponseType.plain)))
            .data ??
        '';
    final mediaPlaylist = await parser.parseString(mediaUri, mediaText);
    if (mediaPlaylist is! HlsMediaPlaylist) {
      throw StateError('Parsed playlist is not a media playlist.');
    }
    ...
    for (int i = 0; i < mediaPlaylist.segments.length; i++) {
      ...
      segments.add(Segment(
          ...
          remoteUri: abs,
          localName: 'seg_${(i + 1).toString().padLeft(5, '0')}.ts'));
    }
    int seqBase = 0;
    try {
    ...
    return HlsParsedResult(
        segments: segments,
        key: key,
        mediaContent: mediaText,
        mediaBase: mediaUri,
        mediaSequenceBase: seqBase);
  }
